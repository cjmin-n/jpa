...
public class ResponseEntityTestController {
    ...
    * */
    ...
    public ResponseEntity findAllUsers(){
        ...
        headers.setContentType(new MediaType("application", "json", Charset.forName("UTF-8"))); // 기본적인 설정으로 바뀔때만 넣어주면된다.

        // 응답에 포함할 데이터 맵을 생성
        Map<String, Object> responseMap = new HashMap<>();
        responseMap.put("users", users);

        return new ResponseEntity(responseMap, headers, HttpStatus.OK); // HttpStatus.OK -> 200
    }

    @GetMapping("/users/{userNo}")
    public ResponseEntity findUserByNo(@PathVariable(name = "userNo") int userNo){
        // http 헤더 객체 생성 - 필수 아님
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(new MediaType("application", "json", Charset.forName("UTF-8"))); // 기본적인 설정으로 바뀔때만 넣어주면된다.
