...
public class SimpleJPQLTests {
     ...
     * - getResultList() : 결과가 2행 이상일 경우 사용하며 컬렉션을 반환한다.
     * 결과가 없으면 빈 컬렉션을 반환한다.
     * */
    ...
    void TypedQuery를_이용한_단일행_조회_테스트() {
        ...
        String jpql = "SELECT m FROM menu_section01 AS m WHERE m.menuCode = 7"; // 전체 조회 시  * 대신 별칭을 넣어줘야 한다.
        TypedQuery<Menu> query = entityManger.createQuery(jpql, Menu.class);

        Menu foundMenu = query.getSingleResult();
        Assertions.assertEquals(foundMenu.getMenuCode(), 7);
    ...
    void distinct_활용한_중복제거_여러행_조회_테스트() {
        ...
        TypedQuery<Integer> query = entityManger.createQuery(jpql, Integer.class);
