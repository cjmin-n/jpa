...
public class CustomAuthFailureHandler implements AuthenticationFailureHandler {
    ...
    /*
     *  onAuthenticationFailure 메소드가 호출 될 defaultFailureUrl 인 경우 리다이렉션을 수행하는 AuthenticationFailUreHandler
     *  속성이 설정되어 있지 않는 경우 실패를 일으킨 AuthenticationException 의 오류 메시지와 함께 클라이언트에게 401 오류를 응답한다.
     * */
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        JSONObject jsonObject;  // dto 를 따로 안 만들어도 알아서 json 객체로 내보냄
        String failMsg;
